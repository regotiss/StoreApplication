...
public class Sale extends JPanel {
	...
	Sale() {
		...
		bak = new JLabel(new ImageIcon(
				Sale.class.getResource("/images/bak5.jpg")));
		add(bak);
		bill = 0;
		bak.setLayout(null);
		...
		JLabel l2 = new JLabel(new ImageIcon(
				Sale.class.getResource("/images/s.png")));
		...
		p.setBorder(BorderFactory.createTitledBorder("Sale Details"));
		bak.add(p);
		...
		tfsid.setBounds(w - 20, h, 80, 30);
		p.add(tfsid);

		l = new JLabel("Product Id:");
		l.setFont(new Font("Comic Sans MS", Font.BOLD, 18));
		l.setBounds(w + 80, h, 120, 30);
		p.add(l);
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					s.execute("insert into sale1 values(" + (vv1.get(0).get(1))
							+ "," + vv1.get(0).get(2) + "," + bill
							+ ",sysdate)");
		...
		l = new JLabel("Bill No:" + grnno);
		l.setFont(new Font("Comic Sans MS", Font.BOLD, 18));
		l.setBounds(w + 400, h - 50, 100, 30);
		bak.add(l);
	...
	public void display() {
		try {
			...
			System.out.println(rate+" "+tfqu.getText());
